 ...
 * esp_config.h).
 *
 * @note Unlike the other hardware accelerator support functions in
 *esp32/hwcrypto, there is no
 * generic "hwcrypto/bignum.h" header for using these functions without mbedTLS.
 *The reason for this
 * is that all of the function implementations depend strongly upon the mbedTLS
 *MPI implementation.
 */
 ...
 *
 * @note All of X, Y & Z should be less than 4096 bit long or an error is
 *returned.
 *
 ...
 */
int esp_mpi_mul_mpi_mod(mbedtls_mpi *Z, const mbedtls_mpi *X,
                        const mbedtls_mpi *Y, const mbedtls_mpi *M);
