...
#ifndef MGOS_ESP32C6_DISABLE_GPIO_SPI_FLASH_CHECK
#define MGOS_ESP32C6_DISABLE_GPIO_SPI_FLASH_CHECK 0
#endif
...
IRAM static void esp32c6_gpio_isr(void *arg) {
  uint32_t int_st = REG_READ(GPIO_STATUS_REG);
  for (uint32_t i = 0, mask = 1; i < GPIO_PIN_COUNT; i++, mask <<= 1) {
    if ((int_st & mask) == 0 || !s_int_ena[i]) continue;
...
IRAM void mgos_gpio_hal_clear_int(int pin) {
  WRITE_PERI_REG(GPIO_STATUS_W1TC_REG, (1U << pin));
}
...
#if !MGOS_ESP32C6_DISABLE_GPIO_SPI_FLASH_CHECK
  if (pin >= 24 && pin <= 30) {
    LOG(LL_ERROR, ("GPIO%d is used by SPI flash, don't use it", pin));
    /*
     * GPIO24-30 are used for SPI flash on the ESP32-C6 and messing with them
     * causes crashes.
     *
     * Add `MGOS_ESP32C6_DISABLE_GPIO_SPI_FLASH_CHECK: 1` to cdefs to disable
     * this check.
     */
    return false;
  }
...
IRAM void mgos_gpio_write(int pin, bool level) {
  ...
  WRITE_PERI_REG(reg, (1U << pin));
}

IRAM bool mgos_gpio_read_out(int pin) {
  return (REG_READ(GPIO_OUT_REG) & (1U << pin)) != 0;
}
...
void esp32c6_nsleep100_40(uint32_t n);
void esp32c6_nsleep100_80(uint32_t n);
void esp32c6_nsleep100_160(uint32_t n);
...
enum mgos_init_result mgos_gpio_hal_init() {
  ...
  esp_err_t r = gpio_isr_register(esp32c6_gpio_isr, NULL, 0, &s_int_handle);
