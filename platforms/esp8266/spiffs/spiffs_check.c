...
// destroys fs->lu_work
ON_FLASH static s32_t spiffs_object_get_data_page_index_reference(
  spiffs *fs,
...
ON_FLASH static s32_t spiffs_rewrite_page(spiffs *fs, spiffs_page_ix cur_pix, spiffs_page_header *p_hdr, spiffs_page_ix *new_pix) {
  s32_t res;
...
ON_FLASH static s32_t spiffs_rewrite_index(spiffs *fs, spiffs_obj_id obj_id, spiffs_span_ix data_spix, spiffs_page_ix new_data_pix, spiffs_page_ix objix_pix) {
  s32_t res;
  spiffs_block_ix bix;
  int entry;
...
ON_FLASH static s32_t spiffs_delete_obj_lazy(spiffs *fs, spiffs_obj_id obj_id) {
...
ON_FLASH static s32_t spiffs_lookup_check_validate(spiffs *fs, spiffs_obj_id lu_obj_id, spiffs_page_header *p_hdr,
...
ON_FLASH static s32_t spiffs_lookup_check_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix cur_block, int cur_entry,
...
ON_FLASH s32_t spiffs_lookup_consistency_check(spiffs *fs, u8_t check_all_objects) {
...
ON_FLASH static s32_t spiffs_page_consistency_check_i(spiffs *fs) {
  const u32_t bits = 4;
...
ON_FLASH s32_t spiffs_page_consistency_check(spiffs *fs) {
...
ON_FLASH static int spiffs_object_index_search(spiffs *fs, spiffs_obj_id obj_id) {
  u32_t i;
...
ON_FLASH static s32_t spiffs_object_index_consistency_check_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix cur_block,
...
// deleted.
ON_FLASH s32_t spiffs_object_index_consistency_check(spiffs *fs) {
  s32_t res = SPIFFS_OK;
  // impl note:
