...
static void print_fpu_regs(const uint32_t *regs, int off, int n) {
  for (int i = 0, j = off; i < n; i++, j++) {
    if (j % 4 == 0) stm32_uart_dputc('\n');
    stm32_uart_dprintf("  S%d: %s0x%08lx", j, (j < 10 ? " " : ""), regs[i]);
  }
}
#endif

void arm_handle_exception(uint8_t isr_no, struct arm_exc_frame *ef,
  ...
  print_fpu_regs((uint32_t *) (((uint8_t *) rf->d) + sizeof(ef->s)), 16,
                 ARRAY_SIZE(ef->s));
  stm32_uart_dputc('\n');
  stm32_uart_dprintf("FPSCR: 0x%08lx\n", rf->fpscr);
#else
  rf->fpscr = 0;
#endif
  mgos_cd_emit_header();
