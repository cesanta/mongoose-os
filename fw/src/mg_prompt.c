...
static struct mg_prompt_state s_sjp;
...
static void show_prompt(void) {
  ...
  printf("[%u/%d] $ ", (unsigned int) mg_get_free_heap_size(),
...
static void process_js(const char *cmd) {
  ...
  if (err == V7_SYNTAX_ERROR) {
    ...
    mg_print_exception(v7, res, "Exec error");
  } else {
    v7_println(v7, res);
  }
...
void mg_prompt_init(struct v7 *v7, int uart_no) {
  ...
  mg_prompt_init_hal();
  show_prompt();
  mg_uart_set_dispatcher(uart_no, mg_prompt_dispatcher, NULL);
...
void mg_prompt_dispatcher(struct mg_uart_state *us) {
  ...
  while (cs_rbuf_get(rxb, 1, &cp) == 1) {
    ...
    mg_prompt_process_char((char) *cp);

    if (us->dispatcher_cb != mg_prompt_dispatcher) {
      /*
