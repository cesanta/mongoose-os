...
MG_PRIVATE enum v7_err spi_js_ctor(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
  spi_connection conn;
  (void) res;

  rcode = miot_spi_create(v7, &conn);
  if (rcode != V7_OK) {
    goto clean;
  }
...
spi_connection spijs_get_conn(struct v7 *v7, v7_val_t this_obj) {
  return v7_get_ptr(
      v7, v7_get(v7, this_obj, s_spi_conn_prop, sizeof(s_spi_conn_prop) - 1));
}
/*
...
*/
MG_PRIVATE enum v7_err spi_js_txn(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
...
*/
MG_PRIVATE enum v7_err spi_js_tran(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
  uint8_t cmd_bits = 0;
  uint16_t cmd_data = 0;
  ...
  dout_data = v7_get_double(v7, tmp_val);
  ...
  tmp_val = v7_arg(v7, 1);
  if (v7_is_number(tmp_val)) {
    din_bits = v7_get_double(v7, tmp_val) * 8;
    if (din_bits > 32) {
      *res = v7_mk_number(v7, -1);
      goto clean;
    }
  ...
  tmp_val = v7_arg(v7, 2);
  if (v7_is_number(tmp_val)) {
    cmd_bits = v7_get_double(v7, tmp_val);
  }

  /* address */
  tmp_val = v7_arg(v7, 3);
  if (v7_is_number(tmp_val)) {
    addr_data = v7_get_double(v7, tmp_val);
...
MG_PRIVATE enum v7_err spi_js_close(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
  spi_connection conn;
