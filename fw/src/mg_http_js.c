...
struct user_data {
   ...
   *
   * - request (prototype: `mg_http_request_proto`)
   * - response (prototype: `mg_http_response_proto`)
   *
...
MG_PRIVATE enum v7_err Http_createServer(struct v7 *v7, v7_val_t *res) {
  ...
  *res = v7_mk_object(v7);
  v7_set_proto(v7, *res, mg_http_server_proto);
...
static void setup_response_object(struct v7 *v7, v7_val_t response,
  ...
  v7_set_proto(v7, response, mg_http_response_proto);
...
static void trigger_event(struct v7 *v7, v7_val_t obj, const char *ev_name,
  ...
  if (v7_is_callable(v7, cb)) {
    mg_invoke_cb1_this(v7, cb, obj, v7_mk_number(v7, (size_t) c));
  }
}
  ...
  switch (ev) {
    ...
    case MG_EV_HTTP_REQUEST:
      if (v7_is_callable(ud->v7, ud->handler)) {
        ...
        mg_invoke_cb2_this(ud->v7, ud->handler, ud->obj, request, response);
    ...
    case MG_EV_HTTP_REPLY:
      ...
      if (v7_is_callable(ud->v7, ud->handler)) {
        ...
        mg_invoke_cb1_this(ud->v7, ud->handler, ud->obj, response);
        v7_disown(ud->v7, &response);
      }
      ...
      break;
    case MG_EV_TIMER:
      mg_invoke_cb0_this(ud->v7, ud->timeout_callback, ud->obj);
      break;
    case MG_EV_CLOSE:
      if (ud != NULL) {
...
static enum v7_err start_http_server(struct v7 *v7, const char *addr,
  ...
  c = mg_bind_opt(mg_get_mgr(), addr, http_ev_handler, opts);
  if (c == NULL) {
 ...
 */
static enum v7_err mg_url_parse(struct v7 *v7, v7_val_t url_v, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
...
MG_PRIVATE enum v7_err Http_Server_destroy(struct v7 *v7, v7_val_t *res) {
  ...
  for (i = mg_next(mg_get_mgr(), NULL); i != NULL;
       i = mg_next(mg_get_mgr(), i)) {
    if (i->listener == c) {
 ...
 */
WARN_UNUSED_RESULT
static enum v7_err mg_http_request_common(struct v7 *v7, v7_val_t opts,
  ...
  if (v7_is_string(opts)) {
    rcode = mg_url_parse(v7, opts, &opts);
    if (rcode != V7_OK) {
      goto clean;
    }
   ...
   */
  if ((c = mg_connect_opt(mg_get_mgr(), addr, http_ev_handler, copts)) ==
      NULL) {
   ...
   * prototype `mg_http_request_proto`), and set provided callback function.
   */
  c->user_data = ud = (struct user_data *) calloc(1, sizeof(*ud));
  if (ud == NULL) {
    rcode = v7_throwf(v7, "Error", "Out of memory");
  ...
  v7_own(v7, &ud->obj);
  v7_set_proto(v7, ud->obj, mg_http_request_proto);
...
MG_PRIVATE enum v7_err Http_createClient(struct v7 *v7, v7_val_t *res) {
  return mg_http_request_common(v7, v7_arg(v7, 0), v7_arg(v7, 1), res);
}

MG_PRIVATE enum v7_err Http_get(struct v7 *v7, v7_val_t *res) {
  struct mg_connection *c;
  enum v7_err rcode = V7_OK;
  rcode = mg_http_request_common(v7, v7_arg(v7, 0), v7_arg(v7, 1), res);
  if (rcode != V7_OK) {
    goto clean;
  }
...
enum v7_err URL_parse(struct v7 *v7, v7_val_t *res) {
  return mg_url_parse(v7, v7_arg(v7, 0), res);
}
...
void mg_http_api_setup(struct v7 *v7) {
...
#endif
  mg_http_response_proto = V7_UNDEFINED;
  mg_http_request_proto = V7_UNDEFINED;

  /*
   * All values are owned temporarily: static values like
   * `mg_http_server_proto` will be owned later forever, in `mg_http_init()`.
   *
  ...
  mg_http_response_proto = v7_mk_object(v7);
  mg_http_request_proto = v7_mk_object(v7);
  ...
  v7_set(v7, Http, "_resp", ~0, mg_http_response_proto);
  v7_set(v7, Http, "_req", ~0, mg_http_request_proto);
  ...
  v7_set_method(v7, mg_http_server_proto, "destroy", Http_Server_destroy);
  v7_set_method(v7, Http, "createServer", Http_createServer);
  v7_set(v7, Http, "_serv", ~0, mg_http_server_proto);
#endif

  /* Initialize response prototype */
  v7_set_method(v7, mg_http_response_proto, "writeHead",
  ...
  v7_set_method(v7, mg_http_request_proto, "write", Http_request_write);
  v7_set_method(v7, mg_http_request_proto, "end", Http_request_end);
  v7_set_method(v7, mg_http_request_proto, "abort", Http_request_abort);
  v7_set_method(v7, mg_http_request_proto, "setTimeout",
                Http_request_set_timeout);
  v7_set_method(v7, mg_http_request_proto, "on", Http_on);
...
void mg_http_js_init(struct v7 *v7) {
  ...
  mg_http_response_proto = v7_get(v7, Http, "_resp", ~0);
  v7_own(v7, &mg_http_response_proto);
