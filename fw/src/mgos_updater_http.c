      ...
      if (ctx->file_size == 0) {
        ...
        if (hm.resp_code != 200) {
          if (hm.resp_code == 304) {
            ...
            mgos_updater_http_start(ctx, loc->p);
            c->user_data = NULL;
          } else {
...
#endif /* MGOS_ENABLE_UPDATER_POST */

static struct update_context *s_ctx;
struct mg_connection *s_update_request_conn;

static void mgos_update_result_cb(struct update_context *ctx) {
  if (ctx != s_ctx) return;
...
static void mgos_update_start(const char *url, int commit_timeout,
  ...
  s_ctx->result_cb = mgos_update_result_cb;
  mgos_updater_http_start(s_ctx, url);
}

static void mgos_update_timer_cb(void *arg) {
  struct sys_config_update *scu = &get_cfg()->update;
  mgos_update_start(scu->url, scu->commit_timeout,
  ...
  switch (ev) {
    ...
    case MG_EV_HTTP_REQUEST: {
      ...
      if (url != NULL) {
        ...
        mgos_update_start(url, commit_timeout, ignore_same_version);
      } else {
        mg_send_response_line(c, 400,
                              "Content-Type: text/plain\r\n"
                              "Connection: close\r\n");
