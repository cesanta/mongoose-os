...
static void gpio_write_handler(struct mg_rpc_request_info *ri, void *cb_arg,
  ...
  if (!miot_gpio_set_mode(pin, MIOT_GPIO_MODE_OUTPUT)) {
    mg_rpc_send_errorf(ri, 400, "error setting pin mode");
    ri = NULL;
    return;
  }
...
static bool gpio_rm_int_cb(int pin) {
  miot_gpio_int_handler_f old_cb = NULL;
  struct pin_int_info *pii = NULL;
  miot_gpio_remove_int_handler(pin, &old_cb, (void **) &pii);
...
static void gpio_set_int_handler(struct mg_rpc_request_info *ri, void *cb_arg,
  ...
  const char *err = NULL;
  struct pin_int_info *pii = NULL;
  enum miot_gpio_int_mode im = MIOT_GPIO_INT_NONE;
  enum miot_gpio_pull_type pt = MIOT_GPIO_PULL_NONE;
  ...
  if (!miot_gpio_set_button_handler(pin, pt, im, debounce_ms, gpio_int_cb,
                                    pii)) {
