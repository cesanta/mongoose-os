...
void mg_conf_parse_cb(void *data, const char *name, size_t name_len,
  ...
  path++;
  const struct mg_conf_entry *e = mg_conf_find_schema_entry(path, ctx->schema);
  if (e == NULL) {
    LOG(LL_INFO, ("Extra key: [%s]", path));
    return;
  }
  if (e->type != CONF_TYPE_OBJECT &&
      !mg_conf_check_access(mg_mk_str(path), ctx->acl)) {
...
bool mg_conf_parse(const struct mg_str json, const char *acl,
  ...
  return (json_walk(json.p, json.len, mg_conf_parse_cb, &ctx) >= 0 &&
          ctx.result == true);
}
...
struct emit_ctx {
  ...
  bool pretty;
  struct mbuf *out;
  mg_conf_emit_cb_t cb;
  void *cb_param;
};

static void mg_emit_indent(struct mbuf *m, int n) {
  mbuf_append(m, "\n", 1);
  for (int j = 0; j < n; j++) mbuf_append(m, " ", 1);
}

static bool mg_conf_value_eq(const void *cfg, const void *base,
                             const struct mg_conf_entry *e) {
  ...
  switch (e->type) {
    ...
    case CONF_TYPE_OBJECT: {
      for (int i = e->num_desc; i > 0; i--) {
        e++;
        if (e->type != CONF_TYPE_OBJECT && !mg_conf_value_eq(cfg, base, e)) {
          return false;
        }
      }
      return true;
...
static void mg_conf_emit_obj(struct emit_ctx *ctx,
                             const struct mg_conf_entry *schema,
                             int num_entries, int indent) {
  mbuf_append(ctx->out, "{", 1);
  bool first = true;
  for (int i = 0; i < num_entries;) {
    const struct mg_conf_entry *e = schema + i;
    if (mg_conf_value_eq(ctx->cfg, ctx->base, e)) {
    ...
    if (ctx->pretty) mg_emit_indent(ctx->out, indent);
    ...
    switch (e->type) {
      ...
      case CONF_TYPE_OBJECT: {
        mg_conf_emit_obj(ctx, schema + i + 1, e->num_desc, indent + 2);
        break;
      }
    }
    i++;
  ...
  if (ctx->pretty) mg_emit_indent(ctx->out, indent - 2);
  mbuf_append(ctx->out, "}", 1);
}
...
void mg_conf_emit_cb(const void *cfg, const void *base,
  ...
  struct emit_ctx ctx = {.cfg = cfg,
                         ...
                         .cb = cb,
                         .cb_param = cb_param};
  mg_conf_emit_obj(&ctx, schema + 1, schema->num_desc, 2);
  if (cb != NULL) cb(out, cb_param);
  if (out == &m) mbuf_free(out);
}

void mg_conf_emit_f_cb(struct mbuf *data, void *param) {
  FILE **fp = (FILE **) param;
...
bool mg_conf_emit_f(const void *cfg, const void *base,
  ...
  mg_conf_emit_cb(cfg, base, schema, pretty, NULL, mg_conf_emit_f_cb, &fp);
