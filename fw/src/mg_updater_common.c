...
static int finalize_write(struct update_context *ctx) {
  ...
  int ret = mg_upd_file_end(ctx->dev_ctx, &ctx->current_file.fi);
  if (ret < 0) {
  ...
  while (true) {
    switch (ctx->update_status) {
      ...
      case US_WAITING_FILE_HEADER: {
        ...
        enum mg_upd_file_action r =
            mg_upd_file_begin(ctx->dev_ctx, &ctx->current_file.fi);

        if (r == MG_UPDATER_ABORT) {
          ctx->status_msg = mg_upd_get_status_msg(ctx->dev_ctx);
          return -1;
        } else if (r == MG_UPDATER_SKIP_FILE) {
      ...
      case US_WAITING_FILE: {
        ...
        int num_processed =
            mg_upd_file_data(ctx->dev_ctx, &ctx->current_file.fi, to_process);
        if (num_processed < 0) {
          ctx->status_msg = mg_upd_get_status_msg(ctx->dev_ctx);
...
int mg_upd_merge_spiffs(spiffs *old_fs) {
  int ret = -1;
  /*
