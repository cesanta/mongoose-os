...
MG_PRIVATE enum v7_err mg_set_interval_or_timeout(struct v7 *v7, v7_val_t *res,
  ...
  if (!v7_is_callable(v7, v7_arg(v7, 0))) {
    ...
    *res = v7_mk_number(v7, mg_set_js_timer(msecs, repeat, v7, cb));
  }
...
MG_PRIVATE enum v7_err global_setTimeout(struct v7 *v7, v7_val_t *res) {
  return mg_set_interval_or_timeout(v7, res, 0);
}

MG_PRIVATE enum v7_err global_setInterval(struct v7 *v7, v7_val_t *res) {
  return mg_set_interval_or_timeout(v7, res, 1);
}
...
MG_PRIVATE enum v7_err global_clearTimeoutOrInterval(struct v7 *v7,
  ...
  if (v7_is_number(v7_arg(v7, 0))) {
    mg_clear_timer(v7_get_double(v7, v7_arg(v7, 0)));
  }
  return V7_OK;
}
