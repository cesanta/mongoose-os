...
static char *miot_rpc_mqtt_topic_name(const struct mg_str device_id) {
  char *topic = NULL;
...
static bool mg_rpc_channel_mqtt_send_frame(struct mg_rpc_channel *ch,
  ...
  if (nc != NULL) {
    ...
    topic = miot_rpc_mqtt_topic_name(mg_mk_str_n(dst.ptr, dst.len));
    mg_mqtt_publish(nc, topic, 0, MG_MQTT_QOS(0), f.p, f.len);
    free(topic);
    miot_invoke_cb(frame_sent, ch);
    return true;
  }
  return false;
}
...
struct mg_rpc_channel *mg_rpc_channel_mqtt(const struct mg_str device_id) {
  char *topic = miot_rpc_mqtt_topic_name(device_id);
  ...
  miot_mqtt_global_subscribe(mg_mk_str(topic), mg_rpc_mqtt_handler, ch);
  free(topic);
