...
#define MG_MQTT_CONNECT_CB "_cocb"
#define MG_MQTT_MESSAGE_CB "_mecb"
#define MG_MQTT_ERROR_CB "_ercb"
#define MG_MQTT_CLOSE_CB "_clcb"
  ...
  switch (ev) {
    case MG_EV_CONNECT:
      if (*(int *) ev_data == 0) {
        ...
        cb = v7_get(v7, ud->client, MG_MQTT_ERROR_CB, ~0);
    ...
    case MG_EV_MQTT_CONNACK: {
      ...
      if (msg->connack_ret_code == MG_EV_MQTT_CONNACK_ACCEPTED) {
        key = MG_MQTT_CONNECT_CB;
      } else {
        key = MG_MQTT_ERROR_CB;
      }
    ...
    case MG_EV_CLOSE:
       ...
       */
      cb = v7_get(v7, ud->client, MG_MQTT_CLOSE_CB, ~0);
      if (!v7_is_undefined(cb)) {
        mg_invoke_cb0(v7, cb);
      }
 ...
 *
 */
enum v7_err mg_mqtt_connect(struct v7 *v7, v7_val_t *res) {
  ...
  nc = mg_connect(mg_get_mgr(), url_with_port ? url_with_port : url,
                  mqtt_ev_handler);
  if (nc == NULL) {
...
void mg_mqtt_api_setup(struct v7 *v7) {
  ...
  v7_val_t mqtt_connect =
      v7_mk_function_with_proto(v7, mg_mqtt_connect, mqtt_proto);
  v7_val_t mqtt;
