...
static enum v7_err miot_conf_get(struct v7 *v7, v7_val_t *res) {
  v7_val_t obj = v7_arg(v7, 0);
  v7_val_t name = v7_arg(v7, 1);
  *res = v7_mk_undefined();

  const struct miot_conf_ctx *ctx =
      (const struct miot_conf_ctx *) v7_get_user_data(v7, obj);
  ...
  const struct miot_conf_entry *e =
      miot_conf_find_schema_entry(key, ctx->schema);
  ...
  switch (e->type) {
    ...
    case CONF_TYPE_OBJECT: {
      *res = miot_conf_mk_proxy(v7, e, ctx->cfg, ctx->read_only, NULL);
      break;
    }
  }
...
static enum v7_err miot_conf_set(struct v7 *v7, v7_val_t *res) {
  v7_val_t obj = v7_arg(v7, 0);
  v7_val_t name = v7_arg(v7, 1);
...
static enum v7_err miot_conf_keys(struct v7 *v7, v7_val_t *res) {
  v7_val_t obj = v7_arg(v7, 0);
  const struct miot_conf_ctx *ctx =
      (const struct miot_conf_ctx *) v7_get_user_data(v7, obj);
  *res = v7_mk_array(v7);
  ...
  for (int i = 1; i <= ctx->schema->num_desc; i++) {
    const struct miot_conf_entry *e = ctx->schema + i;
...
static int miot_conf_desc(struct v7 *v7, v7_val_t target, v7_val_t name,
...
v7_val_t miot_conf_mk_proxy(struct v7 *v7, const struct miot_conf_entry *schema,
  ...
  handler.get = miot_conf_get;
  handler.set = miot_conf_set;
  handler.own_keys = miot_conf_keys;
  handler.get_own_prop_desc = miot_conf_desc;

  v7_val_t obj = v7_mk_object(v7);
  v7_own(v7, &obj);
  struct miot_conf_ctx *ctx = (struct miot_conf_ctx *) calloc(1, sizeof(*ctx));
  ctx->schema = schema;
  ctx->cfg = cfg;
