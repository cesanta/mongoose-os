...
#if MIOT_ENABLE_JS && MIOT_ENABLE_WS_CLIENT_API
...
*
*/
enum v7_err miot_ws_ctor(struct v7 *v7, v7_val_t *res) {
  ...
  if (v7_is_object(this_obj) && this_obj != v7_get_global(v7)) {
    ...
    nc = mg_connect_ws_opt(miot_get_mgr(), ws_ev_handler, copts, url, proto,
                           extra_headers);
    if (nc == NULL) {
...
MG_PRIVATE enum v7_err WebSocket_send(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
...
MG_PRIVATE enum v7_err WebSocket_close(struct v7 *v7, v7_val_t *res) {
  enum v7_err rcode = V7_OK;
  v7_val_t this_obj = v7_get_this(v7);
  struct mg_connection *nc;
  v7_val_t ncv = v7_get(v7, this_obj, "_nc", ~0);
...
MG_PRIVATE enum v7_err WebSocket_readyState(struct v7 *v7, v7_val_t *res) {
  v7_val_t this_obj = v7_get_this(v7);
  v7_val_t ncv = v7_get(v7, this_obj, "_nc", ~0);
...
void miot_ws_client_api_setup(struct v7 *v7) {
  v7_val_t ws_proto = v7_mk_object(v7);
  v7_val_t ws = v7_mk_function_with_proto(v7, miot_ws_ctor, ws_proto);
  v7_own(v7, &ws);
...
#endif /* MIOT_ENABLE_JS && MIOT_ENABLE_WS_CLIENT_API */
