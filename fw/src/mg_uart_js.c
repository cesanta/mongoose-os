...
struct mg_uart_js_state {
  struct v7 *v7;
  v7_val_t obj;
  struct mg_uart_state *us;
  unsigned int recv_pending : 1;
  unsigned int tx_empty_notified : 1;
};

static struct mg_uart_js_state s_uart_js_state[MG_MAX_NUM_UARTS];
static v7_val_t s_uart_proto;

void mg_uart_js_dispatcher(struct mg_uart_state *us) {
  struct mg_uart_js_state *ujs =
      (struct mg_uart_js_state *) us->dispatcher_data;
...
MG_PRIVATE enum v7_err UART_get(struct v7 *v7, v7_val_t *res) {
  ...
  if (v7_is_number(arg0) && (uart_no == 0 || uart_no == 1)) {
    struct mg_uart_js_state *ujs = &s_uart_js_state[uart_no];
    if (ujs->v7 == NULL) {
