...
static struct mgos_prompt_state s_sjp;
...
static void show_prompt(void) {
  ...
  printf("[%u/%d] $ ", (unsigned int) mgos_get_free_heap_size(),
...
static void process_js(const char *cmd) {
  ...
  if (err == V7_SYNTAX_ERROR) {
    ...
    mgos_print_exception(v7, res, "Exec error");
  } else {
    v7_println(v7, res);
  }
...
void mgos_prompt_init(struct v7 *v7, int uart_no) {
  ...
  if (uart_no < 0 || !get_cfg()->debug.enable_prompt ||
      mgos_uart_get_dispatcher(uart_no) != NULL) {
    return;
  }
  ...
  printf("\n");
  mgos_prompt_init_hal();
  show_prompt();
  mgos_uart_set_dispatcher(uart_no, mgos_prompt_dispatcher, NULL);
...
#if MGOS_ENABLE_RPC && MGOS_ENABLE_RPC_CHANNEL_UART
    const int uart_no = s_sjp.uart_no;
    if (uart_no >= 0 && mgos_rpc_get_global() != NULL) {
...
void mgos_prompt_dispatcher(struct mgos_uart_state *us) {
  ...
  while (cs_rbuf_get(rxb, 1, &cp) == 1) {
    ...
    mgos_prompt_process_char((char) *cp);

    if (us->dispatcher_cb != mgos_prompt_dispatcher) {
      /*
