...
#if MIOT_ENABLE_JS && MIOT_ENABLE_WIFI_API
...
MG_PRIVATE enum v7_err miot_Wifi_setup(struct v7 *v7, v7_val_t *res) {
  ...
  ret = miot_wifi_setup_sta(&cfg);

  if (ret && permanent) {
    struct sys_config *cfg = get_cfg();
    cfg->wifi.sta.enable = 1;
    miot_conf_set_str(&cfg->wifi.sta.ssid, ssid);
    miot_conf_set_str(&cfg->wifi.sta.pass, pass);
  }
...
MG_PRIVATE enum v7_err Wifi_connect(struct v7 *v7, v7_val_t *res) {
  (void) v7;
  *res = v7_mk_boolean(v7, miot_wifi_connect());
  return V7_OK;
}

MG_PRIVATE enum v7_err Wifi_disconnect(struct v7 *v7, v7_val_t *res) {
  (void) v7;
  *res = v7_mk_boolean(v7, miot_wifi_disconnect());
  return V7_OK;
}
...
void miot_wifi_scan_done(const char **ssids, void *arg) {
...
MG_PRIVATE enum v7_err Wifi_scan(struct v7 *v7, v7_val_t *res) {
  ...
  miot_wifi_scan(miot_wifi_scan_done, &s_wifi_scan_cb);
...
MG_PRIVATE enum v7_err Wifi_ready(struct v7 *v7, v7_val_t *res) {
  ...
  if (miot_wifi_get_status() == MIOT_WIFI_IP_ACQUIRED) {
    ...
    if (arg != NULL) {
      ...
      arg->v = cbv;
      v7_own(v7, &arg->v);
      miot_wifi_add_on_change_cb(miot_wifi_ready_js, arg);
    } else {
      ret = 0;
    }
  }
...
void miot_wifi_api_setup(struct v7 *v7) {
  ...
  v7_set_method(v7, s_wifi, "setup", miot_Wifi_setup);
  ...
  v7_set(v7, s_wifi, "CONNECTED", ~0, v7_mk_number(v7, MIOT_WIFI_CONNECTED));
  v7_set(v7, s_wifi, "DISCONNECTED", ~0,
         v7_mk_number(v7, MIOT_WIFI_DISCONNECTED));
  v7_set(v7, s_wifi, "GOTIP", ~0, v7_mk_number(v7, MIOT_WIFI_IP_ACQUIRED));
...
#endif /* MIOT_ENABLE_JS && MIOT_ENABLE_WIFI_API */
