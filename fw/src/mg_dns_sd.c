...
#define MG_MDNS_QUERY_UNICAST 0x8000
#define MG_MDNS_CACHE_FLUSH 0x8000
...
void mg_sd_register_service(struct mg_str service_type,
                            struct mg_str service_name, struct mg_str hostname,
                            uint16_t port, struct mg_str *label_keys,
...
void mg_sd_unregister_service(struct mg_str service_type,
...
static void reply_enumeration_record(const char *name,
   ...
   */
  question->rclass &= ~(MG_MDNS_CACHE_FLUSH);
 ...
 */
const char *mg_sd_default_service_type() {
