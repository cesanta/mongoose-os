...
static const char *test_config(void) {
  ...
  const struct sj_conf_entry *schema = sys_conf_schema();
  ...
  /* Load defaults */
  ASSERT_EQ(sj_conf_parse(mg_mk_str(json1), "*", schema, &conf), true);
  ...
  /* Apply overrides */
  ASSERT_EQ(sj_conf_parse(mg_mk_str(json2), "*", schema, &conf), true);
